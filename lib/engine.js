...
class Orchestra extends Component {
	...
	globalListNames = ['groups', 'plugins', 'categories', 'events', 'channels', 'web_hooks', 'monitors', 'alerts', 'tags'];
	...
	afterUserChange(action, args) {
		...
		if (action == 'user_update') {
			for (var id in this.sockets) {
				var socket = this.sockets[id];
				if (socket.auth && socket.user && (socket.username == username)) {
					socket.admin = !!(args.user && args.user.privileges && args.user.privileges.admin);
				}
			}
		} // user_update
