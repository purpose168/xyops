...
class Scheduler {
	...
	schedulerMinuteTick(dargs) {
		...
		this.events.forEach( function(event) {
			...
			var schedules = triggers.filter( function(trigger) { return trigger.type.match(/^(schedule|single|plugin)$/); } );
			...
			while (cursor < now) {
				...
				self.logScheduler(4, "Auto-launching scheduled event: " + event.id + " (" + event.title + ") for timestamp: " + date.toString(), chosen_trigger );
				self.launchJob(job);
			} // while cursor
	...
	execSchedulerPlugin(plugin, items, callback) {
		...
		var child = cp.exec( child_cmd, child_opts, function(err, stdout, stderr) {
			...
			json.items.forEach( function(result, idx) {
				...
				self.logScheduler(4, "Auto-launching deferred scheduled event: " + item.job.id + " for timestamp: " + date.toString(), {
					plugin: plugin.id
				} );
