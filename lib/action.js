...
class Actions {
	...
	runJobActions(job, conditions, callback) {
		...
		final_actions = final_actions.filter( function(action) {
			...
			switch (action.type) {
				...
				case 'store': key += action.bucket_id; break;
				case 'fetch': key += action.bucket_id; break;
				case 'disable': break;
				case 'snapshot': break;
			}
		...
		// filter out actions that already ran (master recovery)
		final_actions = final_actions.filter( function(action) {
			if ('code' in action) {
				self.logAction(9, "Action already ran, skipping it", { action, job_id: job.id });
				return false;
			}
			return true;
		});
	...
	runJobAction_email(job, action, callback) {
		...
		// prep email template
		var template_name = '';
		if (action.template) template_name = action.template;
		else if (action.condition == 'start') template_name = 'job_start';
		...
		this.loadMultipleUsers( action.users, function(err, users) {
			...
			mail_args.user_content = '';
			if (job.table && job.table.header && job.table.rows) {
				var ent_html = function(text) { return (''+text).replace(/\&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;"); };
				mail_args.user_content += '<table><thead><tr><th>' + job.table.header.map( ent_html ).join('</th><th>') + '</tr></thead><tbody>';
				job.table.rows.forEach( function(row) { mail_args.user_content += '<tr><td>' + row.map( ent_html ).join('</td><td>') + '</td></tr>'; } );
				mail_args.user_content += '</tbody></table>';
			}
			if (job.html && job.html.content) {
				mail_args.user_content += '<p>' + job.html.content + '</p>';
			}
			if (!mail_args.user_content) mail_args.user_content = 'n/a';
			...
			self.sendFancyMail( template_name, mail_args, function(err, body, log) {
				...
				if (log && log.length) {
					...
					log.forEach( function(row) {
						var [ msg, args ] = row;
						if (args.tnx === 'message') return; // skip message body
	...
	runJobAction_web_hook(job, action, callback) {
		...
		if (action.text) hook_data.text += " " + action.text.trim();
	...
	runJobAction_run_event(job, action, callback) {
		...
		if (new_job.workflow && !new_job.workflow.start) new_job.workflow.start = trigger.id;
		...
		async.parallel(
			[
				...
				function(callback) {
					...
					var text = hook_data.text.replace(/https?\:\/\/\S+/ig, '').replace(/\:\s*$/, '').replace(name_match, '');
	...
	runJobAction_suspend(job, action, callback) {
		// suspend job until user intervention
		var self = this;
		if (!callback) callback = noop;
		
		this.appendMetaLog(job, "Suspending job until manual user intervention.");
		this.logAction(6, "Suspending job: " + job.id, action);
		
		// we may be resuming a previous suspension (i.e. master recovery)
		var resume_sus = job.suspended || false;
		job.suspended = true;
		...
		async.parallel(
			[
				function(callback) {
					// email
					if (resume_sus) return callback();
					if (!action.users.length && !action.email) return callback();
					var sub_action = Tools.mergeHashes(action, { type: 'email', template: 'job_suspended' });
				...
				function(callback) {
					// web hook
					if (resume_sus) return callback();
					if (!action.web_hook) return callback();
					var sub_action = Tools.mergeHashes(action, { type: 'web_hook', template: 'job_suspended' });
				...
				function(callback) {
					...
					async.whilst(
						function () {
							return !!job.suspended && !self.shut;
						},
						function (callback) {
							setTimeout( function() { callback(); }, 250 );
						},
						function() {
							// user resumed job or server shutdown
							if (self.shut) return callback();
							
							self.appendMetaLog(job, "Resuming job after user intervention.");
							self.logAction(6, "Resuming job: " + job.id);
							
							action.code = 0;
							action.description = "Successfully suspended and resumed job.";
	...
	runJobAction_tag(job, action, callback) {
		...
		action.code = 0;
		action.description = "Successfully added tags.";
		callback();
	}
	...
	runJobAction_store(job, action, callback) {
		...
		async.series([
			...
			function(callback) {
				...
				// merge into bucket
				Tools.mergeHashInto( bucket_data, info.data || {} );
			...
			function(callback) {
				...
				// apply glob pattern
				matched_files = info.files.filter( function(file) { return matcher(file.filename); } );
				...
				matched_files.forEach( function(file) {
					if (!Tools.findObject( bucket_files, { filename: file.filename } )) num_new_files++;
				} );
				...
				async.eachSeries( matched_files,
					function(file, callback) {
						...
						var stub = Tools.findObject( bucket_files, { filename: file.filename } );
						...
						if (stub) {
							...
							stub = {
								...
								path: storage_key_prefix + '/' + Tools.generateUniqueBase64(32) + '/' + Path.basename(file.path), 
								size: file.size,
								job: job.id,
								server: job.server
							};
						...
						self.storage.copy( file.path, stub.path, callback );
					},
					callback
				); // eachSeries
			},
	...
	runJobAction_fetch(job, action, callback) {
		...
		if (!info.input.data) info.input.data = {};
		if (!info.input.files) info.input.files = [];
		...
		async.series([
			...
			function(callback) {
				...
				self.storage.get( bucket_path + '/data', function(err, bucket_data) {
					...
					Tools.mergeHashInto(info.input.data, bucket_data);
	...
	runJobAction_ticket(job, action, callback) {
		// create ticket for job action
		// action: { ticket_type, ticket_assignees, ticket_tags }
		var self = this;
		if (!callback) callback = noop;
		
		this.logAction(6, "Creating ticket for job action: " + action.condition, action);
		...
		this.generateTicketBody_job(ticket, function(err) {
			if (err) {
				action.code = 'ticket';
			...
			self.unbase.insert( { index: 'tickets', id: ticket.id, data: ticket, fast: true }, function(err) {
				...
				self.logAction(6, "Successfully created ticket: " + ticket.id, ticket);
	...
	runJobAction_plugin(job, action, callback) {
		...
		hook_args.type = 'action'; // wire action
		hook_args.condition = action.condition;
	...
	getJobHookData(job, action) {
		...
		// if job is in progress, use jobDetails, otherwise job object should have everything
		var info = this.jobDetails[job.id] || job;
		...
		if (job.server && this.servers[job.server]) {
			...
			if (this.config.get('hostname_display_strip')) {
				hook_data.nice_hostname = hook_data.nice_hostname.replace( new RegExp(this.config.get('hostname_display_strip')), '' );
			}
		...
		if (info.files && info.files.length) {
			hook_data.links.job_files = info.files.map( function(file) { return `- ${base_app_url}/${file.path}`; } ).join("\n");
		}
		...
		if (!text_template && action.template) text_template = text_templates[ action.template ];
		if (!text_template) text_template = text_templates[ 'job_' + action.condition ];
		...
		hook_data.text = this.config.getPath('client.name') + ' ' + this.messageSub( text_template, hook_data, 'n/a' );
	...
	fireSystemHook(action, orig_data) {
		...
		if (hook) {
			...
			// lookup description if we have a template for it
			var desc_template = this.config.getPath('ui.activity_descriptions.' + action);
			if (desc_template) data.description = Tools.sub( desc_template, data );
			...
			if (hook.url) {
				...
				this.request.json( url, data, hook_opts, function(err, resp, data) {
					// log response
					if (err) self.logAction(5, "System Web Hook Error: " + url + ": " + err);
					else self.logAction(9, "System Web Hook Response: " + url + ": HTTP " + resp.statusCode + " " + resp.statusMessage);
				} );
			} // web hook
			...
			if (hook.shell_exec) {
				...
				var child = cp.exec( cmd, child_opts, function(err, stdout, stderr) {
					if (err) self.logAction(5, "System Shell Hook Error: " + cmd + ": " + err);
					else self.logAction(9, "System Shell Hook Completed", { cmd, stdout, stderr } );
				} );
				
				// Write hook data to child's stdin
				child.stdin.on('error', noop);
				child.stdin.write( JSON.stringify({ xy: 1, ...data }) + "\n" );
				child.stdin.end();
			} // command hook
			
			// add variables for email/ticket
			data.nice_date_time = (new Date(data.epoch * 1000)).toString();
			data.hostID = this.hostID;
			data.extras = '';
			
			if (data.admin || data.username || data.ips || data.useragent) {
				data.extras += "\n### Client Info\n\n";
				data.extras += '- **Username:** ' + (data.admin || data.username) + "\n";
				data.extras += '- **IP Addresses:** ' + ((data.ips || []).join(', ') || 'n/a') + "\n";
				data.extras += '- **User Agent:** ' + (data.useragent || 'Unknown') + "\n";
			}
			if (data.server || data.hostname) {
				data.extras += "\n### Server Info\n\n";
				data.extras += '- **Server ID:** ' + (data.server || 'n/a') + "\n";
				data.extras += '- **Hostname:** ' + (data.hostname || 'n/a') + "\n";
				data.extras += '- **IP Address:** ' + (data.ip || 'n/a') + "\n";
			}
			if (!data.extras) data.extras = ' '; // prevent n/a from appearing
			
			data.json = JSON.stringify( Tools.copyHashRemoveKeys(data, { details:1, description:1, nice_date_time:1, extras:1, text:1, message:1, content:1 }), null, "\t" );
			
			if (hook.email) {
				// send email for activity
				data.email_to = hook.email;
				
				this.sendFancyMail( 'activity', data, function(err, body, log) {
					if (err) self.logAction(5, "System Email Hook Error: " + hook.email + ": " + err, { log });
					else self.logAction(9, "System Email Hook Completed", { log } );
				}); // sendFancyMail
			}
			
			if (hook.ticket) {
				// create ticket for activity
				// hook.ticket: { assignees?, type?, cc? }
				this.loadMailTemplate('activity', function(err, template) {
					if (err) return self.logError('hook', "Failed to load email template: " + err);
					
					var ticket = Object.assign({}, {
						subject: 'Activity Report: ' + data.description,
						type: 'issue',
						status: 'open',
						category: '',
						assignees: [],
					...
					ticket.id = Tools.generateShortID('t');
					ticket.num = self.getState('next_ticket_num') || 1;
					ticket.created = ticket.modified = Tools.timeNow(true);
					ticket.username = 'system';
					...
					// compose body
					ticket.body = self.messageSub( template, data, 'n/a', function(value) {
						if ((typeof(value) == 'string') && (value.length == 0)) return 'n/a';
						else return value;
					} );
					
					self.logAction(5, "Creating ticket for system hook", ticket);
					
					// insert ticket to db
					self.unbase.insert( { index: 'tickets', id: ticket.id, data: ticket, fast: true }, function(err) {
						// record is partially indexed
						if (err) {
							self.logError('ticket', "Failed to insert ticket: " + err);
							return;
						}
