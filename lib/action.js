...
class Actions {
	...
	runJobAction_email(job, action, callback) {
		...
		// prep email template
		var template_file = '';
		if (action.condition == 'start') template_file = 'conf/emails/job_start.txt';
		else if (!job.completed) template_file = 'conf/emails/job_progress.txt';
		...
		this.loadMultipleUsers( action.users, function(err, users) {
			...
			self.getJobLogExcerpt( job, function(err, excerpt) {
				mail_args.log_excerpt = Tools.stripANSI( excerpt || 'n/a' );
		...
		async.parallel(
			[
				...
				function(callback) {
					...
					var hook_data = self.getJobHookData(job, action);
					var text = hook_data.text.replace(/https?\:\/\/\S+/ig, '').replace(/\:\s*$/, '').replace(/^OpsRocket\s+/, '');
	...
	runJobAction_plugin(job, action, callback) {
		...
		child_opts.env['OPSROCKET'] = this.server.__version;
	...
	getJobHookData(job, action) {
		...
		hook_data.text = Tools.sub( text_template, hook_data, false, 'n/a' );
