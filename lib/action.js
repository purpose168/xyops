...
class Actions {
	...
	runJobActions(job, triggers, callback) {
		...
		triggers.forEach( function(trigger) {
			self.logAction(8, "Running job actions for trigger: " + trigger, { job_id: job.id });
			
			var actions = Tools.findObjects( job.actions || [], { trigger, enabled: true } );
			if (actions.length) {
				self.logAction(9, "Found " + actions.length + " actions for " + trigger, actions);
			...
			if (trigger.match(/^\w+$/)) {
				// only update stats for standard (alphanumeric) triggers, not `tag:` ones
				self.updateDailyStat( 'job_' + trigger, 1 );
				if (job.event) self.updateDailyCustomStat( `events.${job.event}.job_${trigger}`, 1 );
				if (job.server) self.updateDailyCustomStat( `servers.${job.server}.job_${trigger}`, 1 );
				if (job.category) self.updateDailyCustomStat( `categories.${job.category}.job_${trigger}`, 1 );
				if (job.plugin) self.updateDailyCustomStat( `plugins.${job.plugin}.job_${trigger}`, 1 );
				...
				if (job.server && self.servers[job.server] && self.servers[job.server].groups) {
					self.servers[job.server].groups.forEach( function(group_id) {
						self.updateDailyCustomStat( `groups.${group_id}.job_${trigger}`, 1 );
					} );
				}
			} // std trigger
		}); // foreach trigger
	...
	runJobAction_email(job, action, callback) {
		...
		this.loadMultipleUsers( action.users, function(err, users) {
			...
			// now load the job log excerpt
			self.getJobLogExcerpt( job, function(err, excerpt) {
				...
				fs.readFile( template_file, '', function(err, template_text) {
					...
					var mail_text = Tools.sub( template_text, mail_args, false, 'n/a', function(value) {
					...
					// send it
					self.sendEmail( mail_text, function(err, body, log) {
						if (err) {
							action.code = 'email';
	...
	runJobAction_web_hook(job, action, callback) {
		...
		this.logAction(9, "Firing job web hook for " + action.trigger + ": " + action.web_hook);
	...
	runJobAction_run_event(job, action, callback) {
		...
		this.logAction(6, "Running event for action: " + action.trigger + ": " + event.title, new_job);
	...
	runJobAction_channel(job, action, callback) {
		...
		this.logAction(6, "Notifying channel for action: " + action.trigger + ": " + channel.title, channel);
		...
		async.parallel(
			[
				...
				function(callback) {
					...
					self.logAction(9, "Firing system shell hook for " + action.trigger + ": " + cmd);
				...
				function(callback) {
					...
					var hook_data = self.getJobHookData(job, action);
					var text = hook_data.text.replace(/https?\:\/\/\S+/ig, '').replace(/\:\s*$/, '').replace(/^Orchestra\s+/, '');
					
					self.logAction(9, "Sending notification to users for " + action.trigger, channel.users);
	...
	runJobAction_plugin(job, action, callback) {
		...
		this.logAction(9, "Firing action Plugin for " + action.trigger + ": " + plugin.id + ": " + child_cmd);
	...
	fireWebHook(id, data, callback) {
		...
		(hook.headers || []).forEach( function(header) {
			opts.headers[ header.name ] = Tools.sub(header.value, data, false, data._fallback);
		} );
