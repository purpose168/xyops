// Fancy Echo / REPL tools for OpsRocket
...
function getComponentColor(name) {
	...
	if (chalk.level >= 2) {
		...
		if (name === 'OpsRocket') color = main_color; // special color for orch cat
...
function ellipsis(text, max_len) {
...
exports.setup = function(server) {
	...
	server.on('master', function() {
		...
		if (server.echo && server.config.get('repl')) {
			...
			if (server.debug && web.config.get('http_debug_bind_local') && is_localhost && !server.config.get('expose')) {
			...
			repl.defineCommand('notify', {
				...
				action: function(cmd) {
					...
					var data = {
						...
						sound: Tools.randArray( server.OpsRocket.sounds ),
						loc: ""
					};
					server.OpsRocket.doUserBroadcastAll('notify', data);
