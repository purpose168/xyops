...
class Jobs {
	...
	checkJobStartLimits(job) {
		...
		if (file_limit && job.input && job.input.files) {
			if (job.input.files.length > file_limit.amount) {
				// Note: 0 means NO FILES ALLOWED
				if (!file_limit.amount) this.abortJob(job, `No input files allowed for this event.`);
				else this.abortJob(job, `Maximum number of allowed input files has been exceeded (${file_limit.amount}).`);
				return false;
			}
			
			var total_size = 0;
			job.input.files.forEach( function(file) { total_size += file.size; } );
			if (file_limit.size && (total_size > file_limit.size)) {
				this.abortJob(job, `Maximum total input file size has been exceeded (${Tools.getTextFromBytes(file_limit.size)}).`);
				return false;
			}
		}
	...
	finishJob(job) {
		...
		this.prepJobLog( job, function() {
			...
			job.actions = job.actions.concat( self.config.getPath('job_universal_actions.' + temp_job_type) );
			
			// check for retry
			if (job.code && ((job.code != 'abort') || job.retry_ok)) self.checkRetryJob(job);
