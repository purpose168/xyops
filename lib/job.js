...
class Jobs {
	...
	monitorJobs() {
		...
		for (var event_id in queues) {
			...
			var job_limit = jobs.length ? Tools.findObject( jobs[0].limits, { type: 'job', enabled: true } ) : null;
	...
	finishJob(job) {
		...
		if (job.tags) job.tags.forEach( function(tag) {
			Tools.findObjects( job.actions || [], { trigger: 'tag:' + tag, enabled: true } ).forEach( function(action) {
		...
		// pull out create_snapshot from job, save for later
		var create_snapshot = job.create_snapshot || false;
		delete job.create_snapshot;
		
		// also possibly create_snapshot if job failed
		if (job.code && !create_snapshot && this.config.get('job_error_snapshots')) create_snapshot = true;
		...
		var combo_job = Tools.mergeHashes( job, this.jobDetails[ job.id ] || {} );
		
		// add disk and net averages
		this.calcAvgDiskNet(combo_job);
		...
		this.unbase.insert( 'jobs', job.id, combo_job, function(err) {
			...
			if (create_snapshot && job.server) {
				// take snapshot BEFORE we finalize job completion and refresh page
				self.createSnapshot(job.server, { source: 'job' }, function(err, id) {
					if (err) self.logError('snapshot', "Failed to take snapshot: " + err);
					finish();
				});
			}
			else finish();
