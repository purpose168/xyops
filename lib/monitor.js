...
class Monitoring {
		...
		monitor_defs.forEach( function(mon_def) {
			...
			if (mon_def.data_match) {
				...
				try { value = self.evalExpr( exp ); }
					...
					self.logMonitor(7, "Monitor expression failed to evaluate: " + err, {
						monitor: mon_def,
						hostname: params.hostname,
						expression: exp
					});
			...
			[
				...
				function(callback) {
					...
					alert_defs.forEach( function(alert_def) {
						...
						self.logMonitor(9, "Checking alert expression for " + params.hostname + '/' + alert_def.id + ": " + exp, {
							alert: alert_def,
							server: server.id,
							hostname: params.hostname,
							expression: exp
	...
	sendAlertNotification(args) {
		...
		if (args.email_to) {
			...
			// send it
			this.sendEmail( 'conf/emails/' + args.template + '.txt', args, function(err, raw_email) {
				if (err) {
	...
	createGroupSnapshot(group_id, params, callback) {
		...
		servers = servers.concat( Object.values(this.serverCache).filter( function(server) {
			return server.groups.includes(group_id);
		} ).map( function(server) {
