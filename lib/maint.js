...
class Maintenance {
	...
	finishInternalJob(id) {
		...
		if (job.email && job.details) {
			...
			// send it
			mail.send( 'conf/emails/internal_job.txt', email_args, function(err, body, log) {
				if (err) self.logError('mail', "Failed to send e-mail: " + job.email + ": " + err, { body } );
				else self.logDebug(7, "Email sent successfully to: " + job.email, { body } );
			});
		} // send email
	...
	dbSearchUpdate(opts) {
		...
		this.startInternalJob( { id: opts.id, title: opts.title, username: opts.username || null, stats: opts.stats, type: 'db', quiet: opts.quiet } );
	...
	dbSearchDelete(opts) {
		...
		async.whilst(
			...
			function(callback) {
				...
				// perform search for current chunk
				self.logDebug(9, "Searching chunk for bulk delete", { id: opts.id, stats: opts.stats });
				opts.stats.pages++;
