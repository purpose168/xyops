...
class Maintenance {
	...
	setupMaint() {
		...
		if (fs.existsSync(this.stats.file)) try {
			...
			if (!this.stats.currentDay.transactions) this.stats.currentDay.transactions = {};
			if (!this.stats.currentDay.servers) this.stats.currentDay.servers = {};
		} catch(e) {;}
	...
	updateDailyServerStat(server_id, key, delta = 1) {
		// update daily server stat, e.g. job_something
		if (!this.master) return;
		if (!this.stats.currentDay.servers) this.stats.currentDay.servers = {};
		if (!this.stats.currentDay.servers[server_id]) this.stats.currentDay.servers[server_id] = {};
		this.stats.currentDay.servers[server_id][key] = (this.stats.currentDay.servers[server_id][key] || 0) + delta;
	}
	...
	expireTransferTokens() {
		...
		for (var id in this.transferTokens) {
			...
			if (token.expires <= now) {
				this.logDebug(9, "Deleting expired transfer token: " + id);
	...
	runMaintenance() {
		...
		async.series([
			...
			function(callback) {
				...
				var job = self.startInternalJob({ title: "Routine storage maintenance", type: 'storage' });
	...
	runTimelineMaintenance(callback) {
		...
		var job = self.startInternalJob({ title: "Routine timeline maintenance", type: 'timeline' });
		...
		this.storage.searchRecords( '*', this.unbase.indexes.servers, function(err, results) {
			...
			if (!records.length) {
				job.finish();
				self.logDebug(7, "No servers found for monitor data maint");
				return callback();
			}
			
			self.logDebug(7, "Scanning " + records.length + " servers for monitoring data");
			var server_idx = 0;
			...
			async.eachSeries( records,
						...
						function(sys, callback) {
							...
							self.storage.listGetInfo( list_path, function(err, info) {
								...
								if (info.length > max_rows) {
									...
									self.logDebug(5, "List " + list_path + " has grown too long, trimming to max: " + max_rows, info);
				...
				function(err) {
					...
					else self.logDebug(6, "Server monitor maint completed");
					callback();
				}
			); // eachSeries
		} ); // searchRecords
	...
	runDBMaintenance(callback) {
		...
		async.eachSeries( Object.keys(db_maint),
			function(index_id, callback) {
				...
				self.logDebug(6, "Performing DB maintenance on: " + index_id, opts);
				
				self.storage.get( path, function(err, info) {
					if (err) return callback(); // not created yet, no problem
					self.logDebug(6, "DB Index Info: " + index_id, info);
					...
					if (info.length > opts.max_rows) {
						...
						if (jobs.length) {
							self.logDebug(6, "DB Index " + index_id + " has internal jobs running, skipping maint", { info, jobs });
						}
						else {
							// go go go background job!
							opts.title = "Routine database maintenance (" + index_id + ")";
							opts.index = index_id;
							opts.query = '*';
						...
						// no maint needed
						self.logDebug(6, "DB Index " + index_id + " does not require maintenance, skipping", info);
					}
	...
	startInternalJob(opts) {
		...
		this.logDebug(5, "Starting new internal job: " + opts.id, opts);
	...
	finishInternalJob(id) {
		...
		job.complete = true;
		job.elapsed = Tools.shortFloat( Tools.timeNow() - job.started );
		
		delete job.update;
		delete job.finish;
		
		this.logDebug(5, "Finishing internal job: " + id, job);
	...
	waitForInternalJobs(callback) {
		...
		if (num_jobs) {
			this.logDebug(3, "Waiting for " + num_jobs + " internal jobs to complete", Object.keys(this.internalJobs));
			...
			async.whilst(
				...
				function() {
					// all jobs gone
					self.logDebug(9, "All internal jobs completed.");
					callback();
				}
			); // whilst
		}
	...
	dbSearchUpdate(opts) {
		...
		this.logDebug(5, "Starting bulk search/update operation", { id: opts.id, index: opts.index, query: opts.query });
		this.startInternalJob( { id: opts.id, title: opts.title, username: opts.username || null, stats: opts.stats, type: 'db' } );
		...
		async.whilst(
			...
			function(callback) {
				...
				self.logDebug(9, "Searching chunk for bulk update", { id: opts.id, stats: opts.stats });
				opts.stats.pages++;
				...
				self.unbase.search( opts.index, opts.query, opts, function(err, data) {
					...
					if (!opts.offset) {
						self.logDebug(9, "Total bulk records found: " + data.total, { id: opts.id, stats: opts.stats });
					...
					async.eachSeries( data.records,
						...
						function(err) {
							...
							if (opts.done) {
								...
								self.logDebug(6, "Requested abort of bulk search/update", { id: opts.id, stats: opts.stats });
								return callback();
							}
							if (opts.restart) {
								// user requested we start over at the beginning
								self.logDebug(6, "Requested restart of bulk search/update", { id: opts.id, stats: opts.stats });
								opts.offset = 0;
								opts.stats.pages = 0;
								opts.stats.count = 0;
			...
			function(err) {
				...
				if (opts.restart) {
					self.logDebug(6, "Requested LATE restart of bulk search/update", { id: opts.id });
					delete opts.restart;
					return self.dbSearchUpdate(opts);
				}
				
				self.logDebug(5, "Bulk search/update complete", { id: opts.id, stats: opts.stats });
	...
	dbSearchDelete(opts) {
		...
		this.logDebug(5, "Starting bulk search/delete operation", { id: opts.id, index: opts.index, query: opts.query });
		this.startInternalJob( { id: opts.id, title: opts.title, username: opts.username || null, stats: opts.stats, type: 'db' } );
		...
		async.whilst(
			...
			function(callback) {
				...
				// perform search for current chunk
				self.logDebug(9, "Searching chunk for bulk delete", { id: opts.id });
				opts.stats.pages++;
				...
				self.unbase.search( opts.index, opts.query, opts, function(err, data) {
					...
					if (!opts.offset) {
						self.logDebug(9, "Total bulk records found: " + data.total, { id: opts.id });
						opts.stats.total = data.total;
					}
					...
					async.eachSeries( data.records,
						function(record, callback) {
							...
							if (opts.iterator) opts.iterator(record, finish);
							else finish();
						},
						function(err) {
							if (err) {
							...
							if (opts.done) {
								// user requested abort
								self.logDebug(6, "Requested abort of bulk search/delete", { id: opts.id, stats: opts.stats });
								return callback();
							}
							if ((data.records.length < opts.limit) || (opts.offset + opts.limit >= data.total)) {
								// reached natural end of records
								self.logDebug(6, "Reached natural end of records", { id: opts.id, stats: opts.stats });
								opts.done = true;
								return callback();
							}
							if (opts.max && (opts.stats.count >= opts.max)) {
								// reached user-defined max deletes
								self.logDebug(6, "Reached max delete limit", { id: opts.id, stats: opts.stats });
								opts.done = true;
								return callback();
							}
			...
			function(err) {
				...
				self.logDebug(5, "Bulk search/delete complete", { id: opts.id, stats: opts.stats });
				if (opts.callback) opts.callback();
			}
		); // async.whilst
	...
	dbSearchDelete_jobs(job, callback) {
	...
	dbSearchDelete_servers(server, callback) {
