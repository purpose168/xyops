...
class Admin {
	...
	api_bulk_search_delete_jobs(args, callback) {
		...
		this.loadSession(args, function(err, session, user) {
			...
			if (!self.requirePrivilege(user, 'delete_jobs')) return;
	...
	api_admin_run_optimization(args, callback) {
		...
		this.loadSession(args, function(err, session, user) {
			...
			if (!db) return self.doError('admin', "The current storage configuration does not require optimization.", callback);
			
			callback({ code: 0 });
			
			var job = self.startInternalJob({ title: "Database optimization", username: user.username || user.key, type: 'maint' });
	...
	api_admin_stats(args, callback) {
		...
		this.loadSession(args, function (err, session, user) {
			...
			if (sqlite) {
				...
				stats.db.sqlite += fs.existsSync(db_file) ? Math.floor(fs.statSync(db_file).mtimeMs / 1000) : 0;
				
				var wal_file = db_file + '-wal';
				stats.db.sqlite += fs.existsSync(wal_file) ? Math.floor(fs.statSync(wal_file).mtimeMs / 1000) : 0;
			}
	...
	api_admin_import_data(args, callback) {
		...
		this.loadSession(args, function(err, session, user) {
			...
			var job = self.startInternalJob({ title: "Custom data import", username: user.username || user.key, type: 'maint' });
				...
				function(callback) {
					...
					Tools.fileEachLine( temp_file, { buffer_size: 32768 },
						function(line, callback) {
							...
							catch (e) { self.logError('import', "Failed to parse JSON in file import: " + err + " (line #" + line_idx + ")"); }
							if (!row) return callback();
							
							if (row.index && row.id && row.record) {
								// unbase insert
								self.unbase.insert( row.index, row.id, row.record, function(err) {
									if (err) self.logError('import', "Import line #" + line_idx + " failed: Insert: " + err);
									callback();
								} ); // unbase.insert
							}
								...
								self.storage.put( row.key, row.value, function(err) {
									if (err) self.logError('import', "Import line #" + line_idx + " failed: Put: " + err);
									callback();
								} ); // storage.put
							}
								...
								self.storage[row.cmd].apply( self.storage, row.args.concat( function(err) {
									if (err) self.logError('import', "Import line #" + line_idx + " failed: " + row.cmd + ": " + err);
									callback();
								} ) );
							}
							else {
								self.logError('import', "Import line #" + line_idx + " failed: Unknown format");
								callback();
							}
						}, 
						callback
	...
	api_admin_export_data(args, callback) {
		...
		this.loadSession(args, function(err, session, user) {
			...
			var job = self.startInternalJob({ title: "Custom data export", username: user.username || user.key, type: 'maint' });
			...
			exportTypes.list = function(item, callback) {
				...
				self.storage.get( item.key, function(err, list) {
					...
					// make sure export contains instructions to pre-delete list, for re-import
					stream.write(
						JSON.stringify({ cmd: 'listDelete', args: [item.key, true] }) + "\n" + 
	...
	api_admin_delete_data(args, callback) {
		...
		this.loadSession(args, function(err, session, user) {
			...
			var job = self.startInternalJob({ title: "Custom data deletion", username: user.username || user.key, type: 'maint' });
			var deleteTypes = {};
			...
			deleteTypes.index = function(item, callback) {
				...
				self.dbSearchDelete({
					index: item.index,
					query: item.query || '*',
					title: "Custom bulk deletion (" + item.index + ")",
					username: user.username,
					callback: callback
				});
			}; // delete index
				...
				self.storage.listEach( 'global/users',
					function(user, idx, callback) {
						...
						self.storage.delete( user_path, function(err, user) {
							if (err) {
								// user deleted?
								self.logError('export', "Failed to delete user: " + user_path + ": " + err );
								return callback();
							}
			...
			async.eachSeries( params.items,
				function(item, callback) {
					...
					if (!deleteTypes[item.type]) {
						self.logError('delete', "Unknown item type: " + item.type);
						return callback();
					}
				...
				function() {
					...
					async.eachSeries( self.globalListNames,
						...
						function() {
							job.finish();
							self.logDebug(6, "Data deletion finished", params);
