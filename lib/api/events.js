...
class Events {
	...
	api_run_event(args, callback) {
		...
		if (args.params.json && (typeof(args.params.json) == 'string')) {
			try { Tools.mergeHashInto( args.params, JSON.parse(args.params.json) ); }
			catch (err) { return this.doError('api', "Failed to parse JSON: " + err, callback); }
			delete args.params.json;
		}
		...
		this.loadSession(args, function(err, session, user) {
			...
			if (job.workflow && !job.workflow.start) job.workflow.start = trigger.id;
